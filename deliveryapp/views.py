...
from .models import Meal, Order, OrderDetails, Driver
...
def restaurant_edit_meal(request, meal_id):
    get_meal = Meal.objects.get(id=meal_id)
    form = MealForm(instance=get_meal)

    if request.method == "POST":
        form = MealForm(request.POST, request.FILES, instance=get_meal)
...
def restaurant_order(request):
    user = request.user
    if request.method == "POST":
        order = Order.objects.get(id = request.POST["id"], restaurant=user.restaurant)
